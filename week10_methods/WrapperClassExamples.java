package week10_methods;

import java.sql.SQLOutput;

public class WrapperClassExamples {
    public static void main(String[] args) {

        int i1 = 100;

        Integer i2 = 11; //Auto-boxing: from primitive to object Type

        //explicit example
        Long i3 = (long) 11;

        Integer n1 = 300;

        int n2 = n1; //un-boxing: from object to primitive

        String str = "123";
        System.out.println(str + 1);

        int num = Integer.parseInt(str);
        System.out.println(num + 1); // 124

        String price = "$27.16";
        price = price.substring(1);

        double priceInDouble = Double.parseDouble(price);
        Double priceIınWrapperDouble = Double.valueOf(price);

        System.out.println(priceInDouble);

        // difference between Integer.Parseint(str) and Integer.valueOf(Str)

        Integer num2 = Integer.valueOf(str);

        System.out.println("-------------------------------");

        char ch = 'a';

        boolean isDigit = Character.isDigit(ch);
        System.out.println("isDigit = " + isDigit);

        boolean isLetter = Character.isLetter(ch);
        System.out.println("isLetter = " + isLetter);

        System.out.println("-------------------------------");

        String strTwo = "a1b2c3";

        // sum of the integers inside strTwo.

        int sumOfDigits = 0;

        for (char each : strTwo.toCharArray()) {
            if (Character.isDigit(each)) {
                sumOfDigits += Integer.parseInt(each + "");

            }
        }

        System.out.println("sumOfDigits = " + sumOfDigits);

        System.out.println("--------------------------");

        String strThree = "abced1234!&%()^?589ABCD";

        String letters = " ";
        String uppercaseLetters = " ";
        String lovercaseLetters = "";
        String digits = " ";
        String specialChars = " ";

        for (char each : strThree.toCharArray()) {
            if (Character.isLetter(each)) {
                letters += each;
                if (Character.isUpperCase(each)) {
                    uppercaseLetters += each;
                } else {
                    lovercaseLetters += each;
                }
            } else if (Character.isDigit(each)) {
                digits += each;
            } else {
                specialChars += each;
            }
        }
        System.out.println("letters = " + letters);
        System.out.println("specialChars = " + specialChars);
        System.out.println("lovercaseLetters = " + lovercaseLetters);
        System.out.println("uppercaseLetters = " + uppercaseLetters);
        System.out.println("digits = " + digits);
    }
        }





